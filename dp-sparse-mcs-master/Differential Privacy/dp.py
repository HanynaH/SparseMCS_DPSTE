...
P = [] #obfuscation matrix P[i,j] is the probability from cell i to cell j (generated by linear programming from GLPK这是一个求解线性规划的套件)
...
MeanDiff = []
Uncertainty = [] #uncertainty matrix: Uncertainty[i,j] is the uncertainty from obfuscating cell i to cell j (for linear regression, it is RSE残差的标准差 or (1-Rsquared)（1-R方）)
...
def create_sample_weight():
	...
	for i in range(NUM_OF_CELLS):
		...
		#P.T[i,:]是P每一行的第i列组成的[[a,b,c,...]],为啥非要除这个数呢？？？？！！！！！
		# print('=============test============')
		# print(Uncertainty[:,i])
		# print(P.T[i, :].sum())
		# print(1.0/P.T[i, :].sum())
	...
	SampleWeight025 = list(range(NUM_OF_CELLS))
	SampleWeight05 = list(range(NUM_OF_CELLS))
	SampleWeight075 = list(range(NUM_OF_CELLS))
...
def main(argv):
	...
	for opt, arg in opts:
		if opt == '-h':
			print('usage: dp.py -k <sensed cell/cycle> -s <obfuscation style> -e <e^epsilon> -d <elta> -l <loss function> -t <dataset>'.p)
	...
	global NUM_OF_CELLS, ground_truth_file
	global IS_PM25
